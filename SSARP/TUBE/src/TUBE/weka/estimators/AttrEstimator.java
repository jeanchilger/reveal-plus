...
/*
 *    AttrEstimator.java
 *    Copyright (C) 2006 Gabi Schmidberger
 */
...
/** 
 <!-- globalinfo-start -->
 *
 * Abstract class for all attribute estimators.
 *
 <!-- globalinfo-leftEnd -->
 ...
 */
public abstract class AttrEstimator 
implements Cloneable, Serializable, OptionHandler, CapabilitiesHandler{
  ...
  /** data with one attributes value & original index & maybe class */
  // contains all instances
  protected Instances m_data = null;
 
  /** contains flag if instance is valid or not, not valid if missing. Is parallel to sorted data*/
   protected boolean [] m_valid;
  
  /** attribute index */
  int m_attrIndex = -1;
  
  /** The minimalvalue of training data */
  protected double m_MINValue;
  
  /** The maximalvalue of training data */
  ...
  /**
   * Initialize with a new data set 
   ...
   */
  public void initializeNewData(Instances data, int attrIndex,
      double minValue, double maxValue) throws Exception{
    getCapabilities().testWithFail(data);
    //m_MINValue = m_data.instance(0).value(0);
    //m_MAXValue = m_data.instance(m_numInst - 1).value(0);
    m_MINValue = minValue;
    m_MAXValue = maxValue;

    m_attrIndex = attrIndex;
    
    m_data = MultiBinningUtils.makeOneAttDataset(data, attrIndex, true);
    
    
      //String fname = new String("/home/gs23/attr"+attrIndex+".arff");
      //PrintWriter output = new PrintWriter(new FileOutputStream(fname));
      //output.println(m_data.toString()); 
      //output.close();
      //dbo.dpln(m_dataModel.toString());
    

    //dbo.pln("AttrEst-init\n"+m_data);
    m_valid = MultiBinningUtils.makeFirstValidList(m_data, true);
    m_numMissing = MultiBinningUtils.getNumMissing(m_data);
 
    m_numInst = m_data.numInstances() - m_numMissing;
  }
  
  /**
   * Returns the width of the attribute range
   * @return the width of the attribute range
   */
  ...
  /**
   * Returns the minimum of the attribute range
   * @return the minimum of the attribute range
   */
  ...
  /**
   * Returns the maximum of the attribute range
   * @return the maximum of the attribute range
   */
  ...
  /**
   * Returns the number of not-missing attribute values
   * @return the number of not-missing attribute values
   */
  ...
  /**
   * Returns the dataset build in this attribute estimator
   * @returns the dataset build in this attribute estimator
   */
  ...
  /**
   * Returns the valid flag list
   * @return the valid flag list
   */
 ...
 /**
  ...
  public void addValue(double data, double weight) {
    try {  
      ...
      "Estimator should implement IncrementalAttrEstimator.");
    } catch (Exception ex) {
      ex.printStackTrace();
      System.out.println(ex.getMessage());
    }
  ...
  public void addValues(Instances data, int attrIndex) throws Exception {
    ...
    initializeNewData(data, attrIndex);
    
    // factor is 1.0, data set has not been reduced
    addValues(data, attrIndex, m_MINValue, m_MAXValue, 1.0);
  } */
  ...
  /**
   * Get the value at this offset.
   * @param offset in the dataset
   * @return the value at this offset
   */
  protected double getValue(int offset) {   
    int aIndex = 0;
    return m_data.instance(offset).value(aIndex);
  }
   ...
   * command options
   *
  public static void buildEstimator(AttrEstimator est, String [] options,
  ...
  /*
  public static void buildEstimator(AttrEstimator est,
    ...
    if (!isIncremental) {
      ...
      while (enumInsts.hasMoreElements()) {
        ...
        ((IncrementalAttrEstimator)est).addValue(instance.value(attrIndex),
            instance.weight());
      }
    }
  }
  */
   ...
   *
  private static void setGeneralOptions(Builder build, AttrEstimator est, 
      String [] options)  
  throws Exception {
    Reader input = null;
   ...
   */
  public static AttrEstimator clone(AttrEstimator model) throws Exception {
   ...
   * @exception Exception if an error occurs
   */
  public static AttrEstimator makeCopy(AttrEstimator model) throws Exception {
    
    return (AttrEstimator)new SerializedObject(model).getObject();
  }
   ...
   * @exception Exception if an error occurs
   */
  public static AttrEstimator [] makeCopies(AttrEstimator model,
      int num) throws Exception {
    
    if (model == null) {
      throw new Exception("No model estimator set");
    }
    AttrEstimator [] estimators = new AttrEstimator [num];
    SerializedObject so = new SerializedObject(model);
    for(int i = 0; i < estimators.length; i++) {
      estimators[i] = (AttrEstimator) so.getObject();
    }
    return estimators;
  }
  ...
  public boolean equals(Object obj) {
    ...
    AttrEstimator cmp = (AttrEstimator) obj;
   ...
   */
  public static AttrEstimator forName(String name,
      String [] options) throws Exception {
    
    return (AttrEstimator)Utils.forName(AttrEstimator.class,
        name,
        options);
  }

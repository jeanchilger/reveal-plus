 ...
 * hyperrectangular subspace clusters and Gaussian subspace distributions.
 * <p/>
 <!-- globalinfo-end -->
 *
 <!-- options-start -->
...
public class SubspaceCluster 
  ...
  /** default min range as string */
  protected String m_standardMinString;
  
  /** default max range as string */
  protected String m_standardMaxString;

  /** default min range */
  protected double m_standardMin;
  
  /** default max range */
  protected double m_standardMax;
  
  /** if standard ranges are set */
  private boolean m_standardRangesSet = false;
   ...
   * 
   * <pre> -s &lt;min,max&gt;
   *  The range for standard range. Values will be randomly distributed.</pre>
   *
   * <pre> -b &lt;range&gt;
   *  The indices for boolean attributes.</pre>
   * 
  ...
  public String getStandardRange() { 
    return " "+m_standardMinString+","+m_standardMaxString; 
  }
   ...
   * @param newNoiseRate new percentage of noise 
   */
  public void setStandardRange(String minmaxStr) {
    String minStr = "";
    String maxStr = "";

    // Split the rangeList up into min and max value
    if (!minmaxStr.equals("")) {
      // get min value
      String m = minmaxStr.trim();
      int commaLoc = minmaxStr.indexOf(',');
      if (commaLoc != -1) {
	minStr = minmaxStr.substring(0, commaLoc).trim();
	maxStr = minmaxStr.substring(commaLoc + 1).trim();
      } else {
	minStr = "";
	maxStr = "";
      }
     }
    m_standardMinString = minStr;
    m_standardMaxString = maxStr;   
  }
  ...
  public Instances defineDataFormat() throws Exception {
    ...
    if ((m_standardMinString != null) && 
	(m_standardMaxString != null)) {
      if ((m_standardMinString.length() > 0) && 
	  (m_standardMaxString.length() > 0)) {
	m_standardMax = Double.parseDouble(m_standardMaxString);
	m_standardMin = Double.parseDouble(m_standardMinString);
	m_standardRangesSet = true;
      }
    }

    if (!m_standardRangesSet) {
      checkCoverage();
    }
      ...
      for (int i = 0; i < m_NumAttributes; i++) {
        if (attributes[i]) {
          ...
          if (!m_standardRangesSet) {
            ...
            (m_standardMax - m_standardMin) + m_standardMin;
            if (makeInteger)
              value = Math.round(value);
            example.setValue(i, value);          
          }

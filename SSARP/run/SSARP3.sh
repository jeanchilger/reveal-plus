...
if [ $x -le 2 ]; then
    ...
    cp $file_original  /tmp/final_treina.arff
    exit;
fi
...
./updateRows.pl lac_train_TUBE.txt lac_train_TUBEfinal.txt $vez
...
while [[ $i -le 1 ]]; do
    echo " roda o ALAC ...."
  ../../run_alac_repeated.sh lac_train_TUBEfinal.txt $vez
...
cp alac_lac_train_TUBEfinal.txt  bkppairs
#cat alac_lac_train_TUBE.txt* | awk '{ print $1 }' |  while read instance; do  sed -i  "/^$instance /d" $file_original  ;  done

./scriptRemoveRows.pl alac_lac_train_TUBEfinal.txt composite_train_uniqB$numfeatures composite_train_uniqBold$numfeatures $vez
...
cat composite_train_uniqB$numfeatures  | awk '{ print $1 }'  | while read instance; do  sed  -n  "$instance"p  $file_original; done > /tmp/final_treina.arff;

cut -d' ' -f1 /tmp/final_treina.arff > /tmp/label

...
#----------------------------------------------------------------------
# identify positive and negative documents
#----------------------------------------------------------------------
cat $1 \
    | sort \
    | uniq \
...
cat $1 \
    ...
    | join - goldendb -v1 \
    | cut -d' ' -f1 \
    | sed -e 's/^/-1 /' > temp_negat.$N.$TOPIC

#----------------------------------------------------------------------
# produce the training set file used by SSARP
#----------------------------------------------------------------------
cat temp_posit.$N.$TOPIC temp_negat.$N.$TOPIC \
    | sort -k2 \
    | join - ../"$file".svm.fil.svd  -2 1 -1 2 > trainset.$N.$TOPIC
...
#----------------------------------------------------------------------
# compute positive and negative documents
#----------------------------------------------------------------------
cat ssarp$N.$TOPIC \
    | sort -k 2 \
    | uniq \
...
cat ssarp$N.$TOPIC \
    ...
    | join - goldendb -v1 \
    | cut -d' ' -f1 \
    | sed -e 's/^/-1 /' > x_negat_ssarp_end.$N

cat x_posit_ssarp_end.$N x_negat_ssarp_end.$N \
    | sort \
    | uniq > ssarpout$N.$TOPIC

cut -d' ' -f2  ssarpout$N.$TOPIC > sub_new_ssarp.$N.$TOPIC

e_secondary "script_prune_pairs report:"
echo -e "\tPositive collected ..... `wc -l < x_posit_ssarp_end.$N`"
echo -e "\tNegative collected ..... `wc -l < x_negat_ssarp_end.$N`"

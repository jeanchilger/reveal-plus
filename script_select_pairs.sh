...
#     
    ...
    if [ $len -le 0 ]; then
        ...
        echo "positivos `cat out_after_ssarp.$TOPIC | cut -d' ' -f1 | sort -k1 |  uniq | join - goldendb |  wc -l` total `wc -l < out_after_ssarp.$TOPIC` input $totalPairsInput perda $perda_ac " >> log_run
        exit
    ...
    for i in $(seq $mid -1 0 ); do    
        ...
        echo "$name pos  $pos neg  $neg  real posit $real_posit  perda $perda_ac" >> log_run
        ...
        recall=`echo "scale=6; ($r / $total)" | bc`
        precisao=`echo "scale=6; ($r / $finalpares)" | bc`
        echo "****final result AM is $TOPIC $r ------$finalpares  Recall $recall  Precisao $precisao  posit  $pos $net labellingEffort `wc -l < out_after_ssarp.$TOPIC` perda_ac $perda_ac " >> log_run
        echo "flag $flag" >> log_run

        if [ $pos -le 0 ]; then
                echo "increase flag value" >> log_run
                flag=$(($flag+1))
        else
                echo "new flag equal zero" >> log_run
                flag=0
        fi
        ...
        if [ $flag -eq $sliding_windows ]; then
            new_mid=$i;
            echo "break because flag sliding windows" >> log_run
            break;
        fi
    ...
    if [ $new_mid -le 0 ]; then
        echo "######inside active plus $len_plus" >> log_run
        for i in $(seq $len_plus -1 0 ); do
            name=`printf "data_plus.$TOPIC/xxx%06d" $i`
            echo "inside active plus $name ---- $i" >> log_run
            j=$(($j+1))
            temp=$(($temp+1))
            cat $name | sort > temp
		...
		if [ $j -ge 2000 ]; then
                        echo "##break because of stopping $i" >> log_run
			break;
		fi            
        done
    fi
